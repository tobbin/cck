    ...
    */
    ...
    {                                                     
       ...
       */     
         ...
         * 
         */
        public function payInstallment($chit_id, $customer_id, $installment_id, $amount, $installment_trans_id, $fine=null, $discount=null)
        {
            ...
            $amount_after_fine = $amount - ($fine-$discount);
           
            if($installment_details['payment_status'] != 'C'){     
                $amount_to_pay = $installment_details['amount_to_pay'];
                //payment status 
                $payment_status = ($amount_after_fine >= $amount_to_pay)?'C':'R';
                $amount_for_pay = ($amount_after_fine >= $amount_to_pay)? $amount_to_pay : $amount_after_fine;
                            
                //check if pay by installments                
                if($installment_details['payment_status'] == 'R'){
                    $installment_payment_id = $installment_details['payment_id'];
                    $amount_for_pay += $installment_details['amount_received'];  
                    $fine += $installment_details['fine'];
                    $discount += $installment_details['discount'];
                    $data_array = array('id'=>$installment_payment_id, 'amount_received'=> $amount_for_pay, 'total_bonus'=>0, 'fine'=> $fine, 'discount'=> $discount, 'status'=> $payment_status);     
                    $objInstallmentPayment->updateInstallment($data_array);
                    
                }  else {
                    $data_array = array('customer_id'=>$customer_id, 'chit_id'=>$chit_id,'installment_id'=>$installment_id, 'amount_received'=>$amount_for_pay, 'total_bonus'=>0, 'fine'=> $fine, 'discount'=> $discount, 'status'=>$payment_status);     
                    $installment_payment_id = $objInstallmentPayment->saveAll($data_array);
                }    
                $values = array('installment_payment_id'=>$installment_payment_id, 'installment_transaction_id'=>$installment_trans_id,'amount'=>$amount);     
                $objPaymentTransactionId->saveAll($values);
                $amount = $amount-$amount_for_pay;                
